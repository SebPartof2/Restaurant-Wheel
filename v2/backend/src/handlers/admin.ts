...
export async function handleAddToWhitelist(c: Context<{ Bindings: Env; Variables: RequestContext }>) {
  try {
    ...
    // Add email to whitelist (creates placeholder user if doesn't exist)
    await oauthService.addToWhitelist(email);
...
/**
 * DELETE /api/admin/users/whitelist
 * Remove email from whitelist (admin only)
 */
export async function handleRemoveFromWhitelist(c: Context<{ Bindings: Env; Variables: RequestContext }>) {
  try {
    ...
    await oauthService.removeFromWhitelist(sanitizedEmail);
